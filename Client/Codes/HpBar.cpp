...
HRESULT CHpBar::Ready_GameObject(void)
{
...
void CHpBar::Render_GameObject(void)
{
...
HRESULT CHpBar::Ready_Component(void)
{
	...
	// For.Texture
	pComponent = m_pTextureCom = (Engine::CTexture*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_STATIC, L"Component_Texture_RedGage");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CHpBar::SetUp_OnContantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CHpBar * CHpBar::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	...
	{
		MSG_BOX("CHpBar Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CHpBar::Free(void)
{
	_ulong		dwRefCnt = 0;
