...
HRESULT CEnding::Ready_GameObject(void)
{
	...
	m_fSizeX = _float(BACKSIZEX / 2.f);
	m_fSizeY = _float(BACKSIZEY / 2.f);
...
void CEnding::Render_GameObject(void)
{
...
HRESULT CEnding::Ready_Component(void)
{
	...
	// For.Texture
	pComponent = m_pTextureCom = (Engine::CTexture*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_STATIC, L"Component_Texture_End");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CEnding::SetUp_OnContantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
	...
	m_pTextureCom->SetUp_OnShader(pEffect, "g_BaseTexture", iIdx);
...
CEnding * CEnding::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	...
	{
		MSG_BOX("CEnding Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CEnding::Free(void)
{
	_ulong		dwRefCnt = 0;
