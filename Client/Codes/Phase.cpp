...
CPhase::CPhase(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CPhase::Ready_GameObject(const _float& fTexNum, SCENETYPE eType)
{
	m_fTextureNum = fTexNum;
	m_eType = eType;

	if (FAILED(Ready_Component()))
		return E_FAIL;

	m_fX = _float(BACKSIZEX / 2.f);
	//m_fY = _float(BACKSIZEY / 2.f - 150.f);
	m_fY = 0.f;
...
_int CPhase::Update_GameObject(const _float & fTimeDelta)
{
	if (m_bIsDead)
		return 0;
	...
	if(m_fY < _float(BACKSIZEY / 2.f - 150.f))
		m_fY += 40.f * fTimeDelta;
...
void CPhase::Render_GameObject(void)
{
...
HRESULT CPhase::Ready_Component(void)
{
	...
	// For.Texture
	pComponent = m_pTextureCom = (Engine::CTexture*)CComponent_Manager::GetInstance()->Clone_Component(m_eType, L"Component_Texture_Phase");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CPhase::SetUp_OnContantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
	...
	m_pTextureCom->SetUp_OnShader(pEffect, "g_BaseTexture", m_fTextureNum);
...
CPhase * CPhase::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _float& fTexNum, SCENETYPE eType)
{
	CPhase *	pInstance = new CPhase(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject(fTexNum, eType)))
	{
		MSG_BOX("CPhase Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CPhase::Free(void)
{
	_ulong		dwRefCnt = 0;
