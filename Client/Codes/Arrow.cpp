...
HRESULT CArrow::Ready_GameObject(_vec3* vStartPos, _vec3* vDir, _vec3* vRot, SCENETYPE eType)
{
	m_eType = eType;
...
HRESULT CArrow::SetUp_OnConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CArrow::Collision_ToTarget()
{
	...
	for (; iter != iter_end; ++iter)
	{
		...
		if (m_pTransformCom->Compute_Distance(&pCollider_Monster->Get_Position()) < 5.f)
		{
			...
			{
				dynamic_cast<CMonster*>((*iter))->Set_Hit(15.f);
				m_bIsDead = true;
				return;
			}
		}
	}
	if (m_eType == SCENE_STAGE)
	{
		if (nullptr != CObject_Manager::GetInstance()->Find_Layer(SCENE_STAGE, L"Layer_Santa"))
		{
			auto& iter = CObject_Manager::GetInstance()->Find_Layer(SCENE_STAGE, L"Layer_Santa")->GetObjList()->front();
			CTransform*		pCollider_Santa = (CTransform*)(dynamic_cast<CSanta*>(iter)->Get_Component(L"Com_Transform"));
			if (nullptr == pCollider_Santa)
				return;

			CComponent*		pCollider = (dynamic_cast<CSanta*>(iter)->Get_Component(L"Com_Collider"));

			if (true == m_pColliderCom->Collision_AABB((CCollider*)pCollider))
			{
				dynamic_cast<CSanta*>(iter)->Set_Super();
				m_bIsDead = true;
				return;
			}
		}
...
CArrow * CArrow::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 * vStartPos, _vec3* vDir, _vec3* vRot, SCENETYPE eType)
{
	CArrow*	pInstance = new CArrow(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject(vStartPos, vDir, vRot, eType)))
	{
