#include "stdafx.h"
#include "..\Headers\MainBack_Logo.h"

#include "Component_Manager.h"

CMainBack_Logo::CMainBack_Logo(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CMainBack_Logo::Ready_GameObject(void)
{
	if (FAILED(Ready_Component()))
		return E_FAIL;

	m_fX = _float( BACKSIZEX / 2.f);
	m_fY = _float(BACKSIZEY / 2.f);

	m_fSizeX = _float(BACKSIZEX / 3.f);
	m_fSizeY = _float(BACKSIZEY / 3.f);

	return NOERROR;
}

_int CMainBack_Logo::Update_GameObject(const _float & fTimeDelta)
{
...
void CMainBack_Logo::Render_GameObject(void)
{
...
HRESULT CMainBack_Logo::Ready_Component(void)
{
	...
	// For.Texture
	pComponent = m_pTextureCom = (Engine::CTexture*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_LOGO, L"Component_Texture_MainBackLogo");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CMainBack_Logo::SetUp_OnContantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CMainBack_Logo * CMainBack_Logo::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CMainBack_Logo *	pInstance = new CMainBack_Logo(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		MSG_BOX("CMainBack_Logo Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CMainBack_Logo::Free(void)
{
	_ulong		dwRefCnt = 0;
