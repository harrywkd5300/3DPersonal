#include "stdafx.h"
#include "..\Headers\DoorIn.h"

#include "component_Manager.h"
#include "Object_Manager.h"

CDoorIn::CDoorIn(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CDoorIn::Ready_GameObject()
{
	if (FAILED(Ready_Component()))
		return E_FAIL;


	m_pTransformCom->Set_Position(_vec3(62.f, 1.f, 63.f));
	m_pTransformCom->Rotation_Z(90.f);
	m_pTransformCom->Scaling(_vec3(4.f, 4.f, 4.f));
	return NOERROR;
}

_int CDoorIn::Update_GameObject(const _float & fTimeDelta)
{
	if (m_bIsDead)
		return 1;
...
_int CDoorIn::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (m_bIsLastDead)
		return 1;
...
void CDoorIn::Render_GameObject(void)
{
...
HRESULT CDoorIn::Ready_Component()
{
	...
	// For.Texture
	pComponent = m_pTextureCom = (Engine::CTexture*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_STAGE, L"Component_Texture_DoorIn");
	if (nullptr == pComponent)
		return E_FAIL;
	...
	//Component_Buffer_Bullet
	//For.Buffers
	//pComponent = m_pBufferCom = (Engine::CStatic_Mesh*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_STAGE, L"Component_Buffer_Bullet");
	//if (nullptr == pComponent)
	//	return E_FAIL;
	//m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Com_Buffer", pComponent));
	//m_pBufferCom->AddRef();
...
HRESULT CDoorIn::SetUp_OnConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CDoorIn::Collision_ToPlayer()
{
}

CDoorIn * CDoorIn::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CDoorIn*	pInstance = new CDoorIn(pGraphicDev);
