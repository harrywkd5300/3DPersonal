...
HRESULT CSkillIcon::Ready_GameObject(_vec3* vPos, const _uint& iIdx)
{
...
_int CSkillIcon::Update_GameObject(const _float & fTimeDelta)
{
	if (m_fNotUpdate)
		return 0;
...
void CSkillIcon::Render_GameObject(void)
{
...
HRESULT CSkillIcon::Ready_Component(void)
{
	...
	// For.Texture
	pComponent = m_pTextureCom = (Engine::CTexture*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_STATIC, L"Component_Texture_Skill");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CSkillIcon::SetUp_OnContantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CSkillIcon * CSkillIcon::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3* vPos, const _uint& iIdx)
{
	CSkillIcon *	pInstance = new CSkillIcon(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject(vPos, iIdx)))
	{
		MSG_BOX("CSkillIcon Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CSkillIcon::Free(void)
{
	_ulong		dwRefCnt = 0;
