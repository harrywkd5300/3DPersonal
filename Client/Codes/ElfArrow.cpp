...
CElfArrow::CElfArrow(LPDIRECT3DDEVICE9 pGraphicDev)
	: CGameObject(pGraphicDev)
	, m_pTransformCom(nullptr), m_pBufferCom(nullptr), m_pColliderCom(nullptr)
	, m_pRendererCom(nullptr), m_pShaderCom(nullptr), m_pTargetPlayer(nullptr)
	, m_vDir(0.f, 0.f, 0.f), m_fDist(0.f)
{
}

HRESULT CElfArrow::Ready_GameObject(_vec3* vStartPos, _vec3* vDir, _vec3* vRot)
{
...
_int CElfArrow::Update_GameObject(const _float & fTimeDelta)
{
	if (m_fDist > 20.f)
		return 1;
	...
	m_pTransformCom->Move_ToDirection(m_vDir, 10.f, fTimeDelta);
...
_int CElfArrow::LastUpdate_GameObject(const _float & fTimeDelta)
{
	return 0;
}

void CElfArrow::Render_GameObject(void)
{
...
HRESULT CElfArrow::Ready_Component()
{
	...
	// For.Buffers
	pComponent = m_pBufferCom = (Engine::CStatic_Mesh*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_STAGE, L"Component_Mesh_Arrow");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CElfArrow::SetUp_OnConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CElfArrow::Coll_ToPlayer()
{
	...
	//		return;
	//	if (true == m_pColliderCom->Collision_OBB((CCollider*)pCollider_Shiled))
	//	{
	//		dynamic_cast<CShield*>(m_pShield)->();
	//		dynamic_cast<CDarkElfArcher*>(m_pMonster)->Set_DeadShield();
	//	}
	//}
}

CElfArrow * CElfArrow::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 * vStartPos, _vec3* vDir, _vec3* vRot)
{
	CElfArrow*	pInstance = new CElfArrow(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject(vStartPos, vDir, vRot)))
	{
