...
HRESULT CMpBar::Ready_GameObject(void)
{
	if (FAILED(Ready_Component()))
		return E_FAIL;

	m_fX = 87.f;
	m_fY = _float(BACKSIZEY - 107.f);
...
void CMpBar::Render_GameObject(void)
{
...
HRESULT CMpBar::Ready_Component(void)
{
	...
	// For.Texture
	pComponent = m_pTextureCom = (Engine::CTexture*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_STATIC, L"Component_Texture_BuleGage");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CMpBar::SetUp_OnContantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CMpBar * CMpBar::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CMpBar *	pInstance = new CMpBar(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		MSG_BOX("CMpBar Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CMpBar::Free(void)
{
	_ulong		dwRefCnt = 0;
