...
HRESULT CAim::Ready_Component(void)
{
	...
	// For.Texture
	pComponent = m_pTextureCom = (Engine::CTexture*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_STATIC, L"Component_Texture_Aim");
	if (nullptr == pComponent)
		return E_FAIL;
...
void CAim::Fix_MousePointer(const _uint & iX, const _uint & iY)
{
...
HRESULT CAim::SetUp_OnContantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CAim * CAim::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	...
	{
		MSG_BOX("CAim Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CAim::Free(void)
{
	_ulong		dwRefCnt = 0;
