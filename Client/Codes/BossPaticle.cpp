...
HRESULT CBossPaticle::Ready_GameObject()
{
...
_int CBossPaticle::Update_GameObject(const _float & fTimeDelta)
{
	if (m_bIsDead)
		return 1;
	...
	//_vec3 vMin = m_pTargetTrans->Get_Position() - _vec3(500.f, 500.f, 500.f);
	//_vec3 vMax = m_pTargetTrans->Get_Position() + _vec3(500.f, 500.f, 500.f);
...
_int CBossPaticle::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (m_bIsLastDead)
		return 1;
...
void CBossPaticle::Render_GameObject(void)
{
	...
	//m_pSnowPaticleCom->Paticle_Render();
...
HRESULT CBossPaticle::Ready_Component()
{
...
HRESULT CBossPaticle::SetUp_OnConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CBossPaticle * CBossPaticle::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
