...
void CRectUI_Texture::Set_Pos_Buffer(const _float & fWinSizeX, const _float & fWinSizeY, const _float & fSizeX, const _float & fSizeY)
{
	VTXTEX_VIEWPORT*			pVertex = nullptr;

	m_pVB->Lock(0, 0, (void**)&pVertex, 0);

	pVertex[0].vPosition = _vec4((fWinSizeX - fSizeX / 2.f), (fWinSizeY - fSizeY / 2.f), 0.0f, 1.f);
	pVertex[0].vTexUV = _vec2(0.0f, 0.f);

	pVertex[1].vPosition = _vec4((fWinSizeX + fSizeX / 2.f), (fWinSizeY - fSizeY / 2.f), 0.0f, 1.f);
	pVertex[1].vTexUV = _vec2(1.f, 0.f);

	pVertex[2].vPosition = _vec4((fWinSizeX + fSizeX / 2.f), (fWinSizeY + fSizeY / 2.f), 0.0f, 1.f);
	pVertex[2].vTexUV = _vec2(1.f, 1.f);

	pVertex[3].vPosition = _vec4((fWinSizeX - fSizeX / 2.f), (fWinSizeY + fSizeY / 2.f), 0.0f, 1.f);
	pVertex[3].vTexUV = _vec2(0.f, 1.f);
